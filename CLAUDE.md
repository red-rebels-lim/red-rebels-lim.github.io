...
| `scrape.yml` | manual (`workflow_dispatch`) | runs scraper, sends notifications, opens PR with updated `events.ts` | not on a schedule; NEVER writes to D1 — the database is human-managed (dashboard/MCP), no script writes to it |
