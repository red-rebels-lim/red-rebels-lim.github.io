import type { Player } from '@/types/players';

/**
 * Official Nea Salamina 2026/27 squad (first season back in First Division).
 * Source: transfermarkt.com/nea-salamina-famagusta (squad + transfers, 2026-07-17);
 * 2025/26 entries kept with active:false so historical stats keep resolving.
 *
 * `aliases` cover every name form encountered in events.ts (different scraper sources
 * use Greek-uppercase, Latin transliteration, abbreviated, or Spanish-formal variants).
 * The football-stats resolver maps any alias back to this player's `key`.
 *
 * Bio fields (dateOfBirth, nationality) are intentionally left empty for now — fill
 * in from neasalamina.com or transfermarkt when ready. The page degrades gracefully.
 */
export const players: Player[] = [
  // Goalkeepers
  {
    key: 'alberto_varo_lara',
    sport: 'football-men',
    active: true,
    nameEl: 'Alberto Varo Lara',
    nameEn: 'Alberto Varo Lara',
    position: 'GK',
    shirtNumber: 1,
    photoUrl: '/images/players/alberto_varo_lara.webp',
    aliases: ['ALBERTO VARO LARA', 'Alberto Varo'],
  },
  {
    key: 'daniel_stefanov',
    sport: 'football-men',
    active: false,
    nameEl: 'Stefanov Daniel Milenov',
    nameEn: 'Daniel Stefanov',
    position: 'GK',
    shirtNumber: 13,
  },
  {
    key: 'denys_vakulyk',
    sport: 'football-men',
    active: false,
    nameEl: 'Denys Vakulyk',
    nameEn: 'Denys Vakulyk',
    position: 'GK',
    shirtNumber: 16,
    photoUrl: '/images/players/denys_vakulyk.webp',
  },
  {
    key: 'giannis_kalanidis',
    sport: 'football-men',
    active: false,
    nameEl: 'Γιάννης Καλανίδης',
    nameEn: 'Giannis Kalanidis',
    position: 'GK',
    shirtNumber: 98,
    photoUrl: '/images/players/giannis_kalanidis.webp',
  },

  {
    key: 'daniel_kovacs',
    sport: 'football-men',
    active: true,
    nameEl: 'Dániel Kovács',
    nameEn: 'Dániel Kovács',
    position: 'GK',
    subPosition: 'Central',
    dateOfBirth: '1994-01-16',
    nationality: 'Hungary',
    photoUrl: '/images/players/daniel_kovacs.webp',
    joinedDate: '2026-07-01',
  },
  // Defenders
  {
    key: 'alexandros_theocharous',
    sport: 'football-men',
    active: false,
    nameEl: 'Αλέξανδρος Θεοχάρους',
    nameEn: 'Alexandros Theocharous',
    position: 'DEF',
    subPosition: 'Central',
    shirtNumber: 3,
    photoUrl: '/images/players/alexandros_theocharous.webp',
    aliases: ['ΑΛΕΞΑΝΔΡΟΣ ΘΕΟΧΑΡΟΥΣ'],
  },
  {
    key: 'panagiotis_artymatas',
    sport: 'football-men',
    active: true,
    nameEl: 'Παναγιώτης Αρτύματας',
    nameEn: 'Panagiotis Artymatas',
    position: 'DEF',
    subPosition: 'Central',
    shirtNumber: 4,
    photoUrl: '/images/players/panagiotis_artymatas.webp',
    aliases: ['ΠΑΝΑΓΙΩΤΗΣ ΑΡΤΥΜΑΤΑΣ'],
  },
  {
    key: 'alexantros_michail',
    sport: 'football-men',
    active: true,
    nameEl: 'Αλέξαντρος Μιχαήλ',
    nameEn: 'Alexantros Michail',
    position: 'DEF',
    subPosition: 'Central',
    shirtNumber: 5,
    photoUrl: '/images/players/alexantros_michail.webp',
    // Bare surnames (Μιχαήλ etc.) are deliberately not aliased — they collide with
    // common Cypriot surnames on opposing rosters.
    aliases: ['ΑΛΕΞΑΝΔΡΟΣ ΜΙΧΑΗΛ'],
  },
  {
    key: 'antreas_dimitriou',
    sport: 'football-men',
    active: true,
    nameEl: 'Άντρεας Δημητρίου',
    nameEn: 'Andreas Dimitriou',
    position: 'DEF',
    subPosition: 'Wing',
    shirtNumber: 12,
    photoUrl: '/images/players/antreas_dimitriou.webp',
    aliases: ['ΑΝΤΡΕΑΣ ΔΗΜΗΤΡΙΟΥ'],
  },
  {
    key: 'konstantinos_kastanas',
    sport: 'football-men',
    active: false,
    nameEl: 'Κωνσταντίνος Κάστανας',
    nameEn: 'Konstantinos Kastanas',
    position: 'DEF',
    subPosition: 'Central',
    shirtNumber: 15,
    photoUrl: '/images/players/konstantinos_kastanas.webp',
    aliases: ['ΚΩΝΣΤΑΝΤΙΝΟΣ ΚΑΣΤΑΝΑΣ'],
  },
  {
    key: 'thomas_nikolaou',
    sport: 'football-men',
    active: false,
    nameEl: 'Θωμάς Νικολάου',
    nameEn: 'Thomas Nikolaou',
    position: 'DEF',
    subPosition: 'Wing',
    shirtNumber: 21,
    photoUrl: '/images/players/thomas_nikolaou.webp',
    aliases: ['ΘΩΜΑΣ ΝΙΚΟΛΑΟΥ'],
  },
  {
    key: 'dimitris_kyprianou',
    sport: 'football-men',
    active: false,
    nameEl: 'Δημήτρης Κυπριανού',
    nameEn: 'Dimitris Kyprianou',
    position: 'DEF',
    subPosition: 'Wing',
    shirtNumber: 26,
    photoUrl: '/images/players/dimitris_kyprianou.webp',
    aliases: ['ΔΗΜΗΤΡΗΣ ΚΥΠΡΙΑΝΟΥ', 'Demetris Kyprianou'],
  },
  {
    key: 'kostas_charalampous',
    sport: 'football-men',
    active: true,
    nameEl: 'Κώστας Χαραλάμπους',
    nameEn: 'Kostas Charalampous',
    position: 'DEF',
    subPosition: 'Wing',
    shirtNumber: 33,
    photoUrl: '/images/players/kostas_charalampous.webp',
    aliases: ['ΚΩΣΤΑΣ ΧΑΡΑΛΑΜΠΟΥΣ', 'Costas Charalambous'],
  },
  {
    key: 'giorgos_viktoros',
    sport: 'football-men',
    active: true,
    nameEl: 'Γιώργος Βικτώρος',
    nameEn: 'Giorgos Viktoros',
    position: 'DEF',
    subPosition: 'Central',
    shirtNumber: 44,
    photoUrl: '/images/players/giorgos_viktoros.webp',
    aliases: ['ΓΙΩΡΓΟΣ ΒΙΚΤΩΡΟΣ'],
  },

  {
    key: 'vasilios_lampropoulos',
    sport: 'football-men',
    active: true,
    nameEl: 'Βασίλειος Λαμπρόπουλος',
    nameEn: 'Vasilios Lampropoulos',
    position: 'DEF',
    subPosition: 'Central',
    dateOfBirth: '1990-03-31',
    nationality: 'Greece',
    photoUrl: '/images/players/vasilios_lampropoulos.webp',
    joinedDate: '2026-07-11',
  },
  {
    key: 'florent_hanin',
    sport: 'football-men',
    active: true,
    nameEl: 'Florent Hanin',
    nameEn: 'Florent Hanin',
    position: 'DEF',
    subPosition: 'Wing',
    dateOfBirth: '1990-02-04',
    nationality: 'France',
    photoUrl: '/images/players/florent_hanin.webp',
    joinedDate: '2026-07-16',
  },
  {
    key: 'kyriakos_egomitis',
    sport: 'football-men',
    active: true,
    nameEl: 'Κυριάκος Εγκωμίτης',
    nameEn: 'Kyriakos Egomitis',
    position: 'DEF',
    subPosition: 'Central',
    dateOfBirth: '2008-08-09',
    nationality: 'Cyprus',
  },
  // Midfielders
  {
    key: 'giorgos_christodoulou',
    sport: 'football-men',
    active: true,
    nameEl: 'Γιώργος Χριστοδούλου',
    nameEn: 'Giorgos Christodoulou',
    position: 'MID',
    subPosition: 'Defensive',
    shirtNumber: 6,
    photoUrl: '/images/players/giorgos_christodoulou.webp',
    aliases: ['ΓΕΩΡΓΙΟΣ ΧΡΙΣΤΟΔΟΥΛΟΥ', 'Georgios Christodoulou'],
  },
  {
    key: 'jose_artiles',
    sport: 'football-men',
    active: true,
    nameEl: 'Jose Artiles',
    nameEn: 'Jose Artiles',
    position: 'MID',
    subPosition: 'Attacking',
    shirtNumber: 8,
    photoUrl: '/images/players/jose_artiles.webp',
    aliases: ['MANUEL ARTILES ROMERO JOSE'],
  },
  {
    key: 'antreas_frangkos',
    sport: 'football-men',
    active: false,
    nameEl: 'Άντρεας Φράγκος',
    nameEn: 'Andreas Frangkos',
    position: 'MID',
    subPosition: 'Central',
    shirtNumber: 23,
    photoUrl: '/images/players/antreas_frangkos.webp',
    aliases: ['ΑΝΤΡΕΑΣ ΦΡΑΓΚΟΣ', 'Andreas Fragkos', 'Andreas Frangos'],
  },
  {
    key: 'giorgos_pavlidis',
    sport: 'football-men',
    active: false,
    nameEl: 'Γιώργος Παυλίδης',
    nameEn: 'Giorgos Pavlidis',
    position: 'MID',
    subPosition: 'Central',
    shirtNumber: 24,
    photoUrl: '/images/players/giorgos_pavlidis.webp',
    aliases: ['ΓΙΩΡΓΟΣ ΠΑΥΛΙΔΗΣ'],
  },
  {
    key: 'konstantinos_ilia',
    sport: 'football-men',
    active: true,
    nameEl: 'Κωνσταντίνος Ηλία',
    nameEn: 'Konstantinos Ilia',
    position: 'MID',
    subPosition: 'Defensive',
    shirtNumber: 25,
    photoUrl: '/images/players/konstantinos_ilia.webp',
    aliases: ['ΚΩΝΣΤΑΝΤΙΝΟΣ ΗΛΙΑ'],
  },
  {
    key: 'konstantinos_koumparis',
    sport: 'football-men',
    active: false,
    nameEl: 'Κωνσταντίνος Κουμπαρής',
    nameEn: 'Konstantinos Koumparis',
    position: 'MID',
    subPosition: 'Central',
    shirtNumber: 66,
    photoUrl: '/images/players/konstantinos_koumparis.webp',
    aliases: ['ΚΩΝΣΤΑΝΤΙΝΟΣ ΚΟΥΜΠΑΡΗ', 'Konstantinos Koumbari'],
  },

  {
    key: 'alois_confais',
    sport: 'football-men',
    active: true,
    nameEl: 'Aloïs Confais',
    nameEn: 'Aloïs Confais',
    position: 'MID',
    subPosition: 'Defensive',
    dateOfBirth: '1996-09-07',
    nationality: 'France',
    photoUrl: '/images/players/alois_confais.webp',
    joinedDate: '2026-07-02',
  },
  {
    key: 'jordi_quintilla',
    sport: 'football-men',
    active: true,
    nameEl: 'Jordi Quintillà',
    nameEn: 'Jordi Quintillà',
    position: 'MID',
    subPosition: 'Defensive',
    dateOfBirth: '1993-10-25',
    nationality: 'Spain',
    photoUrl: '/images/players/jordi_quintilla.webp',
  },
  {
    key: 'giannis_gerolemou',
    sport: 'football-men',
    active: true,
    nameEl: 'Γιάννης Γερολέμου',
    nameEn: 'Giannis Gerolemou',
    position: 'MID',
    subPosition: 'Central',
    dateOfBirth: '2000-01-27',
    nationality: 'Cyprus',
    photoUrl: '/images/players/giannis_gerolemou.webp',
    joinedDate: '2026-07-01',
    aliases: ['ΓΙΑΝΝΗΣ ΓΕΡΟΛΕΜΟΥ'],
  },
  {
    key: 'andreas_mavroudis',
    sport: 'football-men',
    active: true,
    nameEl: 'Άντρεας Μαυρουδής',
    nameEn: 'Andreas Mavroudis',
    position: 'MID',
    subPosition: 'Attacking',
    dateOfBirth: '2008-06-28',
    nationality: 'Cyprus',
  },
  // Forwards
  {
    key: 'giorgos_pontikou',
    sport: 'football-men',
    active: false,
    nameEl: 'Γιώργος Ποντικού',
    nameEn: 'Giorgos Pontikou',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 7,
    photoUrl: '/images/players/giorgos_pontikou.webp',
    aliases: ['ΓΙΩΡΓΟΣ ΠΟΝΤΙΚΟΥ'],
  },
  {
    key: 'daniel_perez',
    sport: 'football-men',
    active: false,
    nameEl: 'Daniel Perez',
    nameEn: 'Daniel Perez',
    position: 'FWD',
    subPosition: 'Central',
    shirtNumber: 9,
    photoUrl: '/images/players/daniel_perez.webp',
    aliases: ['ALEJANDRO PEREZ CORDOVA DANIEL', 'Daniel Pérez'],
  },
  {
    key: 'mika_borges',
    sport: 'football-men',
    active: false,
    nameEl: 'Mika Borges',
    nameEn: 'Mika Borges',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 10,
    photoUrl: '/images/players/mika_borges.webp',
    // 'Mika Borges' is intentionally not aliased — it duplicates nameEn (auto-included
    // by the resolver) and risks future short-form collisions.
    aliases: ['FILIPE CORREIA BORGES MICAEL', 'Micael Filipe Correia Borges'],
  },
  {
    key: 'alex_guti',
    sport: 'football-men',
    active: false,
    nameEl: 'Alex Guti',
    nameEn: 'Alex Guti',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 11,
    photoUrl: '/images/players/alex_guti.webp',
    aliases: ['ALEJANDRO GUTIERREZ HERNANDEZ', 'Álex Guti'],
  },
  {
    key: 'aldair_djalo_balde',
    sport: 'football-men',
    active: true,
    nameEl: 'Aldair Djalo Balde',
    nameEn: 'Aldair Djalo Balde',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 17,
    photoUrl: '/images/players/aldair_djalo_balde.webp',
    // Bare 'Aldair' is intentionally not aliased — common Lusophone first name,
    // collision risk with future opponents.
    aliases: ['ADULAI DJALO BALDE ALDAIR'],
  },
  {
    key: 'konstantinos_anthimou',
    sport: 'football-men',
    active: false,
    nameEl: 'Κωνσταντίνος Άνθιμου',
    nameEn: 'Konstantinos Anthimou',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 27,
    photoUrl: '/images/players/konstantinos_anthimou.webp',
    aliases: ['ΚΩΝΣΤΑΝΤΙΝΟΣ ΑΝΘΙΜΟΥ'],
  },
  {
    key: 'savvas_kontopoulos',
    sport: 'football-men',
    active: false,
    nameEl: 'Σάββας Κοντόπουλος',
    nameEn: 'Savvas Kontopoulos',
    position: 'FWD',
    subPosition: 'Central',
    shirtNumber: 77,
    photoUrl: '/images/players/savvas_kontopoulos.webp',
    aliases: ['ΣΑΒΒΑΣ ΚΟΝΤΟΠΟΥΛΟΣ'],
  },
  {
    key: 'panagiotis_louka',
    sport: 'football-men',
    active: true,
    nameEl: 'Παναγιώτης Λούκα',
    nameEn: 'Panagiotis Louka',
    position: 'FWD',
    subPosition: 'Central',
    shirtNumber: 89,
    photoUrl: '/images/players/panagiotis_louka.webp',
    aliases: ['ΠΑΝΑΓΙΩΤΗΣ ΛΟΥΚΑ', 'P. Louka'],
  },
  {
    key: 'kyriakos_epifaniou',
    sport: 'football-men',
    active: true,
    nameEl: 'Κυριάκος Επιφανίου',
    nameEn: 'Kyriakos Epifaniou',
    position: 'FWD',
    subPosition: 'Wing',
    shirtNumber: 99,
    aliases: ['ΚΥΡΙΑΚΟΣ ΕΠΙΦΑΝΙΟΥ'],
  },
  {
    key: 'christos_giousis',
    sport: 'football-men',
    active: true,
    nameEl: 'Χρήστος Γιούσης',
    nameEn: 'Christos Giousis',
    position: 'FWD',
    subPosition: 'Wing',
    dateOfBirth: '1999-02-08',
    nationality: 'Greece',
    photoUrl: '/images/players/christos_giousis.webp',
    joinedDate: '2026-07-10',
  },
  {
    key: 'samuel_grandsir',
    sport: 'football-men',
    active: true,
    nameEl: 'Samuel Grandsir',
    nameEn: 'Samuel Grandsir',
    position: 'FWD',
    subPosition: 'Wing',
    dateOfBirth: '1996-08-14',
    nationality: 'France',
    photoUrl: '/images/players/samuel_grandsir.webp',
    joinedDate: '2026-07-01',
  },
  {
    key: 'dario_poveda',
    sport: 'football-men',
    active: true,
    nameEl: 'Darío Poveda',
    nameEn: 'Darío Poveda',
    position: 'FWD',
    subPosition: 'Central',
    dateOfBirth: '1997-03-13',
    nationality: 'Spain',
    photoUrl: '/images/players/dario_poveda.webp',
    joinedDate: '2026-07-13',
  },
  {
    key: 'braydon_manu',
    sport: 'football-men',
    active: true,
    nameEl: 'Braydon Manu',
    nameEn: 'Braydon Manu',
    position: 'FWD',
    subPosition: 'Wing',
    dateOfBirth: '1997-03-28',
    nationality: 'Ghana',
    photoUrl: '/images/players/braydon_manu.webp',
    joinedDate: '2026-07-20',
  },

  /**
   * Volleyball squads — source: neasalamina.com/volley-roster/ (2026-07-20).
   * The page lists no birth dates or nationalities; positions use the club's
   * Greek terms (ΠΑΣΑΔΟΡΟΣ/ΑΚΡΑΙΑ/ΔΙΑΓΩΝΙΟΣ/ΚΕΝΤΡΙΚΟΣ/ΛΙΜΠΕΡΟ). nameEl keeps
   * the script the club uses per player (Greek for locals, Latin for imports).
   */
  // Volleyball women — setters
  {
    key: 'carly_anderson',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Carly Anderson',
    nameEn: 'Carly Anderson',
    position: 'SETTER',
    shirtNumber: 4,
    photoUrl: '/images/players/carly_anderson.webp',
  },
  {
    key: 'alexandra_christou',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Αλεξάνδρα Χριστού',
    nameEn: 'Alexandra Christou',
    position: 'SETTER',
    shirtNumber: 14,
    photoUrl: '/images/players/alexandra_christou.webp',
  },
  // Volleyball women — outside hitters
  {
    key: 'olivia_bell',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Olivia Bell',
    nameEn: 'Olivia Bell',
    position: 'OUTSIDE',
    shirtNumber: 3,
    photoUrl: '/images/players/olivia_bell.webp',
  },
  {
    key: 'konstantina_siapani',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Κωνσταντίνα Σιαπάνη',
    nameEn: 'Konstantina Siapani',
    position: 'OUTSIDE',
    shirtNumber: 8,
    photoUrl: '/images/players/konstantina_siapani.webp',
  },
  {
    key: 'maria_achilleos',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Μαρία Αχιλλέως',
    nameEn: 'Maria Achilleos',
    position: 'OUTSIDE',
    shirtNumber: 12,
    photoUrl: '/images/players/maria_achilleos.webp',
  },
  {
    key: 'savanna_shiaelis',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Σαβάννα Σιαήλη',
    nameEn: 'Savanna Shiaelis',
    position: 'OUTSIDE',
    shirtNumber: 25,
    photoUrl: '/images/players/savanna_shiaelis.webp',
  },
  // Volleyball women — opposites
  {
    key: 'anni_siaga',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Άννη Σιαγά',
    nameEn: 'Anni Siaga',
    position: 'OPPOSITE',
    shirtNumber: 9,
    photoUrl: '/images/players/anni_siaga.webp',
  },
  {
    key: 'julie_oliveira_sousa',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Julie Oliveira Sousa',
    nameEn: 'Julie Oliveira Sousa',
    position: 'OPPOSITE',
    shirtNumber: 16,
    photoUrl: '/images/players/julie_oliveira_sousa.webp',
  },
  {
    key: 'katerina_saveriadou',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Κατερίνα Σαβεριάδου',
    nameEn: 'Katerina Saveriadou',
    position: 'OPPOSITE',
    shirtNumber: 17,
    photoUrl: '/images/players/katerina_saveriadou.webp',
  },
  // Volleyball women — middle blockers
  {
    key: 'ekaterina_ripnaya',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Ekaterina Ripnaya',
    nameEn: 'Ekaterina Ripnaya',
    position: 'MIDDLE',
    shirtNumber: 7,
    photoUrl: '/images/players/ekaterina_ripnaya.webp',
  },
  {
    key: 'eirini_tzioni',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Ειρήνη Τζιώνη',
    nameEn: 'Eirini Tzioni',
    position: 'MIDDLE',
    shirtNumber: 10,
    photoUrl: '/images/players/eirini_tzioni.webp',
  },
  {
    key: 'chrysovalanto_siali',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Χρυσοβαλάντω Σιαλή',
    nameEn: 'Chrysovalanto Siali',
    position: 'MIDDLE',
    shirtNumber: 18,
    photoUrl: '/images/players/chrysovalanto_siali.webp',
  },
  {
    key: 'nikoletta_stylianou',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Νικολέττα Στυλιανού',
    nameEn: 'Nikoletta Stylianou',
    position: 'MIDDLE',
    shirtNumber: 44,
    photoUrl: '/images/players/nikoletta_stylianou.webp',
  },
  // Volleyball women — liberos
  {
    key: 'nefeli_christou',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Νεφέλη Χριστού',
    nameEn: 'Nefeli Christou',
    position: 'LIBERO',
    shirtNumber: 1,
    photoUrl: '/images/players/nefeli_christou.webp',
  },
  {
    key: 'maria_chitziou',
    sport: 'volleyball-women',
    active: true,
    nameEl: 'Μαρία Χιτζιού',
    nameEn: 'Maria Chitziou',
    position: 'LIBERO',
    shirtNumber: 2,
    photoUrl: '/images/players/maria_chitziou.webp',
  },
  // Volleyball men — setters
  {
    key: 'ivan_pliasetskyi',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Ivan Pliasetskyi',
    nameEn: 'Ivan Pliasetskyi',
    position: 'SETTER',
    shirtNumber: 4,
    photoUrl: '/images/players/ivan_pliasetskyi.webp',
  },
  {
    key: 'petr_lebedev',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Petr Lebedev',
    nameEn: 'Petr Lebedev',
    position: 'SETTER',
    shirtNumber: 5,
    photoUrl: '/images/players/petr_lebedev.webp',
  },
  // Volleyball men — outside hitters
  {
    key: 'juan_pablo_montoya',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Juan Pablo Montoya',
    nameEn: 'Juan Pablo Montoya',
    position: 'OUTSIDE',
    shirtNumber: 2,
    photoUrl: '/images/players/juan_pablo_montoya.webp',
  },
  {
    key: 'anninos_chrysostomou',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Άννινος Χρυσοστόμου',
    nameEn: 'Anninos Chrysostomou',
    position: 'OUTSIDE',
    shirtNumber: 7,
    photoUrl: '/images/players/anninos_chrysostomou.webp',
  },
  {
    key: 'giorgos_gurdis',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Γιώργος Γκούρτης',
    nameEn: 'Giorgos Gurdis',
    position: 'OUTSIDE',
    shirtNumber: 16,
    photoUrl: '/images/players/giorgos_gurdis.webp',
  },
  {
    key: 'symeon_topuzliev',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Symeon Topuzliev',
    nameEn: 'Symeon Topuzliev',
    position: 'OUTSIDE',
    shirtNumber: 17,
    photoUrl: '/images/players/symeon_topuzliev.webp',
  },
  // Volleyball men — opposites
  {
    key: 'richard_peemuller',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Richard Peemuller',
    nameEn: 'Richard Peemuller',
    position: 'OPPOSITE',
    shirtNumber: 10,
    photoUrl: '/images/players/richard_peemuller.webp',
  },
  // Volleyball men — middle blockers
  {
    key: 'marios_nikolaou',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Μάριος Νικολάου',
    nameEn: 'Marios Nikolaou',
    position: 'MIDDLE',
    shirtNumber: 9,
    photoUrl: '/images/players/marios_nikolaou.webp',
  },
  {
    key: 'konstantinos_pafitis',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Κωνσταντίνος Παφίτης',
    nameEn: 'Konstantinos Pafitis',
    position: 'MIDDLE',
    shirtNumber: 12,
    photoUrl: '/images/players/konstantinos_pafitis.webp',
  },
  {
    key: 'volodymyr_tarasenko',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Volodymyr Tarasenko',
    nameEn: 'Volodymyr Tarasenko',
    position: 'MIDDLE',
    shirtNumber: 15,
    photoUrl: '/images/players/volodymyr_tarasenko.webp',
  },
  {
    key: 'chrysostomos_efstathiou',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Χρυσόστομος Ευσταθίου',
    nameEn: 'Chrysostomos Efstathiou',
    position: 'MIDDLE',
    shirtNumber: 19,
    photoUrl: '/images/players/chrysostomos_efstathiou.webp',
  },
  // Volleyball men — liberos
  {
    key: 'lefteris_ioannou',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Ελευθέριος Ιωάννου',
    nameEn: 'Lefteris Ioannou',
    position: 'LIBERO',
    shirtNumber: 3,
    photoUrl: '/images/players/lefteris_ioannou.webp',
  },
  {
    key: 'alexey_durunda',
    sport: 'volleyball-men',
    active: true,
    nameEl: 'Alexey Durunda',
    nameEn: 'Alexey Durunda',
    position: 'LIBERO',
    shirtNumber: 8,
    photoUrl: '/images/players/alexey_durunda.webp',
  },
];
