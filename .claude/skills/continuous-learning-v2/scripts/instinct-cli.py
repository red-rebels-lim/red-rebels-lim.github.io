...
def find_project_root() -> Path:
    ...
    if (cwd / ".claude").exists():
        return cwd
    # Ultimate fallback - go up from script location
    return Path(__file__).resolve().parent.parent.parent.parent

PROJECT_ROOT = find_project_root()
HOMUNCULUS_DIR = PROJECT_ROOT / ".claude" / "homunculus"
INSTINCTS_DIR = HOMUNCULUS_DIR / "instincts"
PERSONAL_DIR = INSTINCTS_DIR / "personal"
SHARED_DIR = INSTINCTS_DIR / "shared"
EVOLVED_DIR = HOMUNCULUS_DIR / "evolved"
OBSERVATIONS_FILE = HOMUNCULUS_DIR / "observations.jsonl"

# Ensure directories exist
for d in [PERSONAL_DIR, SHARED_DIR, EVOLVED_DIR / "skills", EVOLVED_DIR / "commands", EVOLVED_DIR / "agents"]:
...
def cmd_import(args):
    ...
    output_file = SHARED_DIR / f"{source_name}-{timestamp}.yaml"
